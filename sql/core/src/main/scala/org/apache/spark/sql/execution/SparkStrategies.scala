...
abstract class SparkStrategies extends QueryPlanner[SparkPlan] {
  ...
  object JoinSelection extends Strategy with JoinSelectionHelper {
    ...
    private def checkDistMapJoinBuildSide(
        onlyLookingAtHint: Boolean,
        buildSide: Option[BuildSide],
        joinType: JoinType,
        hint: JoinHint): Unit = {
      ...
      if (onlyLookingAtHint && buildSide.isEmpty) {
        if (hintToDistMapJoinLeft(hint)) {
          invalidBuildSide(hint.leftHint.get, "left")
        }
        if (hintToDistMapJoinRight(hint)) {
    ...
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      ...
      case j @ ExtractEquiJoinKeys(joinType, leftKeys, rightKeys, nonEquiCond,
        ...
        def createDistributedMapJoin(onlyLookingAtHint: Boolean) = {
          val buildSide = getDistMapJoinBuildSide(
            left, right, joinType, hint, onlyLookingAtHint, conf)
          checkDistMapJoinBuildSide(onlyLookingAtHint, buildSide, joinType, hint)
          buildSide.map {
            buildSide =>
