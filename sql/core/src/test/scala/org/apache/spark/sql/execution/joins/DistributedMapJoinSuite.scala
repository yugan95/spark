...
import org.apache.spark.sql.catalyst.plans.logical.{DISTMAPJOIN, DistMapJoinStrategy, JoinStrategyHint}
  ...
  test("DMJ hint in SQL is applied to correct side") {
    withTempView("t", "u") {
      ...
      val plan3 = sql(
        ...
        .asInstanceOf[org.apache.spark.sql.catalyst.plans.logical.Join]
      assert(plan1.hint.leftHint.get.strategy.exists(isDistMapJoin))
      assert(plan1.hint.rightHint.isEmpty)
      assert(plan2.hint.leftHint.isEmpty)
      assert(plan2.hint.rightHint.get.strategy.exists(isDistMapJoin))
